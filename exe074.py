'''Exercício Python 074: Crie um programa que vai gerar cinco
 números aleatórios e colocar em uma tupla. Depois disso, mostre a
 listagem de números gerados e também indique o menor e o maior valor que estão na tupla.'''

#biblioteca para randomizar, sortear os números
from random import randint
#sorteando os 5
num = (randint(1,10), randint(1,10), randint(1,10), randint(1,10), randint(1,10))
print('Os valores sorteados foram: ', end='')
#percorrendo a tupla e imprimindo os valores
for n in num:
    print(f'{n}',end=' ')
#mostrar maior e menor
print(f'\nO maior valor sorteado foi {max(num)}')
print(f'O menor valor sorteado foi {min(num)}')
